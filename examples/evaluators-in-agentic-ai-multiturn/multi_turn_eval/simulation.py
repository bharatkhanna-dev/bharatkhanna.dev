...
class SimulatedUser:
    ...
    def generate_initial_message(self) -> str:
        """Generate the opening user message for the conversation."""
    ...
    def respond_to(self, agent_response: str) -> str | None:
        """
        Generate the user's next message in response to the agent.
        Returns None when conversation should end (TASK_COMPLETE or TASK_FAILED).
        """
...
class TurnEvaluator:
    ...
    def score(self, task: str, agent_response: str, context: str) -> dict[str, float]:
        """Score a single agent response turn."""
...
def run_simulation(
    ...
    """
    Run a complete multi-turn simulation.

    agent_callable: A function (user_message: str) -> agent_response: str
    Returns: SimulationResult with per-turn scores and aggregate metrics
    """
