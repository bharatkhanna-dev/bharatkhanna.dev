...
Results are printed to stdout. For LangSmith integration, the simulation
results can be pushed as feedback via the SDK (see push_to_langsmith()).
"""
...
def customer_support_agent(user_message: str) -> str:
    """Simple demo agent. Replace with your real agent invocation."""
...
SCENARIOS = [
    {
        "id": "password_reset",
        "persona": (
            "A non-technical office worker in their 50s who forgot their account password. "
            "Not very comfortable with technology. Slightly anxious but polite."
        ),
    ...
    },
    {
        "id": "billing_dispute",
        "persona": (
            "A business owner who was charged twice on their credit card for the same subscription. "
            "Professional but increasingly frustrated after waiting a week for a resolution."
        ),
    ...
    },
    {
        ...
        "persona": (
            "A junior developer trying to integrate the product's API into a Python application. "
            "Has read the documentation but is stuck on authentication."
        ),
        "task": (
            "Find out the correct way to pass an API key for OAuth2 authentication "
            "and get a working example in Python."
        ),
    },
    {
        "id": "adversarial_confused_user",
        "persona": (
            "A user who is extremely confused about what the product does. "
            "They think this is a food delivery service and keep asking about their pizza order."
        ),
...
def push_to_langsmith(result: SimulationResult, project_name: str = "multi_turn_sim") -> None:
    """Push simulation results as feedback to LangSmith."""
    try:
        from langsmith import Client
        import uuid
        client = Client()

        # Create a synthetic run to attach feedback to
        ...
        client.create_feedback(run_id=run_id, key="mean_accuracy", score=result.mean_accuracy)
...
def main() -> None:
    ...
    if (completed / len(results)) < MIN_COMPLETION_RATE:
        print(f"\n  FAIL: Completion rate {completed/len(results):.0%} < threshold {MIN_COMPLETION_RATE:.0%}")
        raise SystemExit(1)
