...
Demonstrates:
  ...
  - Integration of middleware into a LangGraph Stategraph
"""
...
class LoopDetectionMiddleware:
    """
    Tracks per-resource edit counts. Injects a reconsideration prompt
    after the threshold is exceeded, breaking potential doom loops.
    """
        ...
        if resource and tool_name in ("edit_file", "write_file", "update_resource"):
            ...
            if self._counts[resource] >= self.threshold:
                return (
                    ...
                    f"{self._counts[resource]} times. Consider reconsidering your "
                    f"approach — you may be stuck in a loop."
                )
        return None
...
class PreCompletionChecklistMiddleware:
    """
    Detects when the agent is about to mark a task complete and forces
    an explicit verification pass before allowing exit.
    """
...
def call_llm(state: AgentState) -> dict:
    """LLM node — optionally injects verification checklist."""
    messages = state["messages"]
    verification_injected = state.get("verification_injected", False)

    # Inject pre-completion checklist if warranted and not already done
    if (
...
def execute_tools(state: AgentState) -> dict:
    """
    Tool execution node with loop detection middleware.
    Injects warnings into message history when loops are detected.
    """
    last_msg = state["messages"][-1]
    injected_warnings = []
    tool_node = ToolNode(tools)

    # Check for loop warnings before execution
    ...
    # Append warnings as system messages so the LLM sees them on next call
