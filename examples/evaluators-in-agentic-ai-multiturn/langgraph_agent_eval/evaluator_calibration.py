...
# Re-use the judge and calibration_test from llm_as_judge/
...
def main():
    ...
    for d in result["details"]:
        ...
        print(
            f"  {mark}  Q: {d['question'][:42]:<42} "
            f"human={d['human_score']:.1f}  judge={d['judge_score']:.2f}"
        )

    if not result["passed"]:
        print(
            f"\n  Action required: Judge agreement {result['agreement']:.0%} < {threshold:.0%}.\n"
            "  Recalibrate the rubric, or switch judge model, before running production evals."
        )
        sys.exit(1)
